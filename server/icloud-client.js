...
function errorWithStatus(message, status = 502, code = "ICLOUD_MAIL_ERROR") {
...
function allowedHostSet(value) {
  return new Set(String(value || DEFAULT_ALLOWED_HOSTS).split(",").map((item) => item.trim().toLowerCase()).filter(Boolean));
}

function parseAccessUrl(value, email, allowedHosts) {
  let parsed;
  try { parsed = new URL(String(value || "").trim()); } catch {
    throw errorWithStatus("iCloud 取件 URL 无效", 400, "INVALID_ICLOUD_MAIL_URL");
  }
  if (!["http:", "https:"].includes(parsed.protocol) || parsed.username || parsed.password) {
    throw errorWithStatus("iCloud 取件 URL 无效", 400, "INVALID_ICLOUD_MAIL_URL");
  }
  if (!allowedHosts.has(parsed.hostname.toLowerCase())) {
    throw errorWithStatus("iCloud 取件 URL 域名不受信任", 400, "UNTRUSTED_ICLOUD_MAIL_HOST");
  }
  const segments = parsed.pathname.split("/").filter(Boolean);
  let scopedEmail = "";
  try { scopedEmail = decodeURIComponent(segments[2] || ""); } catch { /* handled below */ }
  if (segments.length !== 3 || segments[0] !== "messages" || !/^[a-z0-9_-]{16,}$/i.test(segments[1])
    ...
    throw errorWithStatus("iCloud 取件 URL 与邮箱不匹配", 400, "ICLOUD_MAIL_URL_MISMATCH");
  }
  return parsed.toString();
}

export function parseCredentialLine(value, { allowedHosts = DEFAULT_ALLOWED_HOSTS } = {}) {
  const raw = scalar(value).trim();
  const separator = raw.indexOf("----");
  if (separator <= 0) {
    throw errorWithStatus("iCloud 格式应为：基础邮箱----取件URL", 400, "INVALID_ICLOUD_FORMAT");
  }
  const email = normalizeEmail(raw.slice(0, separator));
  if (!email || email.split("@")[1] !== "icloud.com" || email.split("@")[0].includes("+")) {
    throw errorWithStatus("只支持不带 +tag 的 iCloud 基础邮箱", 422, "UNSUPPORTED_ICLOUD_EMAIL");
  }
...
export class IcloudClient {
  constructor({ db, encryptionKey, fetchFn = fetch, allowedHosts, requestTimeoutMs, timezoneOffset } = {}) {
    this.db = db;
    this.fetch = fetchFn;
    this.allowedHosts = allowedHosts || process.env.ICLOUD_MAIL_ALLOWED_HOSTS || DEFAULT_ALLOWED_HOSTS;
    const configuredTimeout = Number(requestTimeoutMs ?? process.env.ICLOUD_MAIL_REQUEST_TIMEOUT_MS ?? DEFAULT_REQUEST_TIMEOUT_MS);
    this.requestTimeoutMs = Number.isFinite(configuredTimeout) && configuredTimeout > 0
      ? Math.floor(configuredTimeout)
      : DEFAULT_REQUEST_TIMEOUT_MS;
    const offset = String(timezoneOffset || process.env.ICLOUD_MAIL_TIMEZONE_OFFSET || DEFAULT_TIMEZONE_OFFSET);
  ...
  decrypt(value) {
    ...
    if (version !== "v1" || !iv || !tag || !encrypted) {
      throw errorWithStatus("iCloud 取件凭据无法解密", 500, "ICLOUD_MAIL_DECRYPT_FAILED");
    }
  ...
  async jsonRequest(url) {
    ...
    try {
      ...
      if (!response.ok) {
        throw errorWithStatus("iCloud 取件服务请求失败", response.status || 502, "ICLOUD_MAIL_REQUEST_FAILED");
      }
      return data;
    } catch (error) {
      if (error?.code === "ICLOUD_MAIL_REQUEST_FAILED") throw error;
      if (timedOut) throw errorWithStatus(`iCloud 取件请求超时（${this.requestTimeoutMs}ms）`, 504, "ICLOUD_MAIL_REQUEST_TIMEOUT");
      throw errorWithStatus("iCloud 取件服务暂时不可用", 502, "ICLOUD_MAIL_REQUEST_FAILED");
    } finally {
      clearTimeout(timer);
    }
  }
  ...
  async listMessages(accessUrl) {
    ...
    if (!Array.isArray(data?.items)) {
      throw errorWithStatus("iCloud 取件服务返回了无效的邮件列表", 502, "INVALID_ICLOUD_MAIL_RESPONSE");
    }
    return data.items.slice(0, MAX_MESSAGES_PER_SCAN);
  }

  async importCredential(value) {
    const credentials = parseCredentialLine(value, { allowedHosts: this.allowedHosts });
    ...
    if (account && account.provider !== "icloud") {
    ...
    this.db.transaction(() => {
      ...
      audit(this.db, account.id, "account", "iCloud 取件地址导入完成", credentials.email, { provider: "icloud_mail_link" });
    })();
  ...
  async importCredentials(value) {
    ...
    if (!lines.length) throw errorWithStatus("请粘贴至少一行 iCloud 取件格式", 400, "ICLOUD_CREDENTIALS_REQUIRED");
    if (lines.length > 100) throw errorWithStatus("单次最多导入 100 个 iCloud 邮箱", 400, "ICLOUD_IMPORT_LIMIT");
    const items = [];
    ...
    if (!importedItems.length) {
      throw errorWithStatus(items[0]?.error || "iCloud 邮箱导入失败", 422, "ICLOUD_IMPORT_FAILED");
    }
    return {
  ...
  credentialsFor(account) {
    ...
    if (!row) throw errorWithStatus("这个 iCloud 邮箱还没有导入取件地址", 409, "ICLOUD_CREDENTIALS_REQUIRED");
  ...
  normalizeMessage(account, item, detail) {
    ...
    const senderAddress = normalizeEmail(detail?.fromAddress || item?.from_address) || scalar(detail?.fromAddress || item?.from_address).trim();
    ...
    const graphMessageId = `icloud-${messageId}`;
  ...
  async scanInbox(account) {
    if (account.provider !== "icloud") throw errorWithStatus("这个邮箱不是 iCloud 取件邮箱", 409, "OAUTH_PROVIDER_MISMATCH");
