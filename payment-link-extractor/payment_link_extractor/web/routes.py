...
def _config_from_payload(payload: dict[str, Any]) -> ExtractionConfig:
    ...
    country = forced_country or requested_country or str(os.getenv("OPLL_COUNTRY", "DE") or "DE").upper()
    proxy_country = re.search(
        r"-(?:res|country|region|area|dc|res_sc)-([A-Za-z]{2})(?:[-_:]|$)",
        str(checkout_proxy or ""),
    )
    if proxy_country and not forced_country and not requested_country:
