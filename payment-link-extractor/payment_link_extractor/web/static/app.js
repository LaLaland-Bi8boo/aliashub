(function () {
  ...
  const WORKBENCH_BASE_PATH = window.location.pathname.replace(/\/+$/, "");
  const tasks = new Map();
  ...
  function apiFetch(url, options = {}) {
    ...
    const target = url.startsWith("/") ? `${WORKBENCH_BASE_PATH}${url}` : url;
    return fetch(target, { ...options, headers }).then(response => {
  ...
  function connectTaskSocket() {
    ...
    socket = new WebSocket(
      `${protocol}//${window.location.host}${WORKBENCH_BASE_PATH}/ws/tasks`,
    );
